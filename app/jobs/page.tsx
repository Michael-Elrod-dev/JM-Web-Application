  ...
  const handleViewSelected = () => {
    if (selectedJobs.length > 0) {
      const selectedJobNames = selectedJobs.map(id => 
        jobs.find(job => job.job_id.toString() === id)?.job_title
      ).filter(Boolean);
      router.push(`/jobs/active?search=${encodeURIComponent(selectedJobNames.join(', '))}`);
    }
  };
    ...
    <>
      ...
      {filteredJobs.map((job) => (
        <JobFrame 
          ...
          name={job.job_title} 
          overdue={job.overdue_count} 
          nextWeek={job.next_week_count} 
          laterWeeks={job.later_weeks_count} 
