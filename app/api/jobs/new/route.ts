...
export async function POST(request: Request) {
  try {
    ...
    if (!data.client.user_id && (!data.client.user_name || !data.client.user_email)) {
      throw new Error('Either existing client ID or new client information (name and email) is required');
    }
    ...
    try {
      ...
      if (data.client.user_id) {
        ...
        const [result] = await connection.execute<ResultSetHeader>(
          'INSERT INTO app_user (user_type, user_name, user_email, user_phone) VALUES (?, ?, ?, ?)',
          ['Client', data.client.user_name, data.client.user_email, data.client.user_phone || null]
        );
