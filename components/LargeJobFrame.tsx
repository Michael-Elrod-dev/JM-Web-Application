...
import { JobDetailView, PhaseView } from '../app/types/views';
...
const LargeJobFrame: React.FC<JobDetailView> = (props) => {
  ...
  const renderDropdown = (label: string, items: any[]) => (
    <div className="mb-4">
      ...
      <div className="relative">
        <select className="block appearance-none w-full bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 hover:border-zinc-400 dark:hover:border-zinc-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline dark:text-white">
          {items.map((item, index) => (
            <option key={index} value={typeof item === 'string' ? item : item.task_title || item.material_title}>
              {typeof item === 'string' ? item : item.task_title || item.material_title}
            </option>
          ))}
        </select>
  ...
  return (
   <div className="bg-white dark:bg-zinc-800 shadow-md overflow-hidden sm:rounded-lg mb-4 p-6">
     ...
     <div className="my-6">
       ...
       <div className="relative h-6">
         <div className="absolute inset-0 flex rounded-full overflow-hidden">
           ...
           >
             <span className="text-xs font-bold text-white">{props.overdue}</span>
           </div>
           <div 
             className="bg-yellow-500 flex items-center justify-center" 
             style={{width: `${(props.nextSevenDays / total) * 100}%`}}
           >
             <span className="text-xs font-bold text-white">{props.nextSevenDays}</span>
           </div>
           <div 
             className="bg-green-500 flex items-center justify-center" 
             style={{width: `${(props.sevenDaysPlus / total) * 100}%`}}
           >
             <span className="text-xs font-bold text-white">{props.sevenDaysPlus}</span>
           </div>
         </div>
       </div>
     </div>
     ...
     <div className="mt-4">
       <div className="w-full" style={{ height: `${20 + props.phases.length * 28}px` }}>
         <Timeline 
           phases={props.phases}
           startDate={startDate}
           endDate={endDate}
     ...
     {/* TODO: Update modal with S3 image handling when implemented */}
     {isModalOpen && (
       <div 
